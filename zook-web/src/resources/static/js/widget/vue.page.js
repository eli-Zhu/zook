...
Vue.component('vue-page', {
    template: '<ul class="pagination pull-right no-margin"><li v-if="currentPage > 1" ><a v-on:click="paginator.currentPage-- ,pageClick($event)">上一页</a></li><li v-else class="disabled"><a>上一页</a></li><li v-for="index in indexs"  v-bind:class="{\'active\': currentPage === index }"><a v-on:click="btnClick(index, $event)" v-html="index"></a></li><li v-if="currentPage != totalPage" ><a v-on:click="paginator.currentPage++,pageClick($event)">下一页</a></li><li v-else class="disabled"><a>下一页</a></li><li class="disabled"><a>共<i v-html="totalPage"></i>页</a></li></ul>',
